...
try {
    ...
    Write-Host "Stage 1 @ 0x7C00, Stage 1.5 @ 0x$(([int]$STAGE_1_5_LOAD_TARGET).ToString("X")) (which is segment 0x$(([int]$stage1_5Segment).ToString("X"))), Stage 2 @ 0x$(([int]$STAGE_2_LOAD_TARGET).ToString("X")) (which is segment 0x$(([int]$stage2Segment).ToString("X"))), Stage 3 @ 0x$(([int]$STAGE_3_LOAD_TARGET).ToString("X")), Stage 4 @ 0x$(([int]$STAGE_4_LOAD_TARGET).ToString("X")). For a total of 0x$(([int]$neededSectors).ToString("X")) sectors to load from disk."
    ...
    for ($x = 0; $x -lt $kernel64Bytes.Length; $x++ ) { $osBytes[$x + (($stage1Sectors + $stage2Sectors + $kernelSectors + $kernel64Sectors) * 512)] = $stage1_5Bytes[$x] }
