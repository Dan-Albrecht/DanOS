...
try {
    cargo build --release --target .\i686-unknown-none.json -Zbuild-std

    # For now, always handy to have the assembly around
    rust-objdump.exe --disassemble-all .\target\i686-unknown-none\release\kernel > .\target\i686-unknown-none\release\kernel.asm
    ...
    rust-objdump.exe -d .\target\i686-unknown-none\release\kernel.elf > .\target\i686-unknown-none\release\kernel.elf.asm
}
finally {
