use crate::{
    acpi::pciGeneralDevice::PciGeneralDevice, assemblyStuff::halt::haltLoop, vgaWriteLine,
};
...
impl Controller {
    pub fn tryGet(header: *const PciGeneralDevice) -> Option<Controller> {
        unsafe {
    ...
    unsafe fn getXType(&self, index: i32) -> Option<bool> {
        ...
        vgaWriteLine!("Port {} is @ 0x{:X}, status @ 0x{:X} with status 0x{:X}", index, portAddr as usize, statusAddr as usize, status);
