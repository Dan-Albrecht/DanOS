...
try {
    ...
    Write-Host "Assembling with 0x$(([int]$sectorsToLoad).ToString("X")) sector to load, which will be loaded to segment 0x$(([int]$targetMemorySegment).ToString("X")), and we'll hand off to 0x$(([int]$handoffToSegment).ToString("X"))"
    ...
    Write-Host "Finished assembling Stage 1"
}
finally {
