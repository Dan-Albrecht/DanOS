...
try {
    ...
    nasm.exe .\bootloaderStage1.asm -Werror -DMAX_SECTOR_READ_COUNT="$maxSectorReadCount" -DFULL_SECTOR_BLOCKS="$fullBlocks" -DREMAINING_SECTORS="$remainingSectors" -DDISK_DATA_MEMORY_SEGMENT="$targetMemorySegment" -DSTAGE2_TARGET_MEMORY_SEGMENT="$handoffToSegment" -f bin -o .\bootloaderStage1.bin
