...
try {
    ...
    qemu-system-x86_64 -machine type=q35 -drive id=disk,file=./build/DanOS.img,format=raw,if=none -device ahci,id=ahci -device ide-hd,drive=disk,bus=ahci.0 -monitor stdio -S -gdb tcp::3333 -d cpu_reset
    # In WSL, find host IP
